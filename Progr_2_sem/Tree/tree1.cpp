...
void result(tree*& tr) {  //  Функция удаления четных элементов
    if (tr) {  //  Идем по всему дереву
        tree* p = tr;  //  Создаём указатель на голову
        result(tr->left);  //  Проходимся по левому поддереву
        result(tr->right);  //  Проходимся по правому поддереву
        if (p->inf % 2 == 0 && p->inf) Delete(tr, p); //  Если узел существует и он четный, то удаляем его
    }
}
int main(){
